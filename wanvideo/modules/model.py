...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def block_swap(self, blocks_to_swap, offload_txt_emb=False, offload_img_emb=False, vace_blocks_to_swap=None):
        logging.info(f"Swapping {blocks_to_swap + 1} transformer blocks")
        ...
        for b, block in tqdm(enumerate(self.blocks), total=len(self.blocks), desc="Initializing block swap"):
            block_memory = 5124 #get_module_memory_mb(block)
        ...
        logging.info("----------------------")
        logging.info(f"Block swap memory summary:")
        logging.info(f"Transformer blocks on {self.offload_device}: {total_offload_memory:.2f}MB")
        logging.info(f"Transformer blocks on {self.main_device}: {total_main_memory:.2f}MB")
        logging.info(f"Total memory used by transformer blocks: {(total_offload_memory + total_main_memory):.2f}MB")
        logging.info(f"Non-blocking memory transfer: {self.use_non_blocking}")
        logging.info("----------------------")
